    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      {filteredQuestions.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200/80 p-12 text-center shadow-2xs">
          ...
          <h3 className="text-base font-extrabold text-slate-800">Không tìm thấy câu hỏi phù hợp bộ lọc</h3>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 bg-slate-50/50">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold text-xs text-slate-900 bg-white border border-slate-200 px-3 py-1 rounded-xl shadow-2xs font-mono">
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            <div className="text-slate-900 text-base leading-relaxed font-normal bg-slate-50/50 p-6 rounded-2xl border border-slate-200/70">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {currentQuestion.options.map((opt) => {
                    ...
                    let cardStyle = 'border-slate-200 bg-white hover:border-indigo-300 hover:bg-slate-50/60 text-slate-800';
                    let badgeStyle = 'bg-slate-100 text-slate-700 border-slate-200';

                    if (isChecked) {
                      if (isCorrectOpt) {
                        cardStyle = 'border-emerald-500 bg-emerald-50/80 text-emerald-950 ring-2 ring-emerald-500/20';
                        badgeStyle = 'bg-emerald-600 text-white border-emerald-600';
                      } else if (isWrongSelected) {
                        cardStyle = 'border-rose-400 bg-rose-50/80 text-rose-950 ring-2 ring-rose-500/20';
                        badgeStyle = 'bg-rose-500 text-white border-rose-500';
                      }
                    } else if (isSelected) {
                      cardStyle = 'border-indigo-600 bg-indigo-50/80 text-indigo-950 ring-2 ring-indigo-500/20';
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-4 rounded-xl border text-left transition-all flex items-start gap-3.5 shadow-2xs ${cardStyle}`}
                      >
                ...
                <div className="flex items-center gap-3 max-w-md">
                  ...
                  {!isChecked && (
                    <button
                      ...
                      className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-medium text-xs rounded-xl shadow-xs transition-colors"
                    >
                      Xác nhận
                    </button>
                  )}
            ...
            {!isChecked && (
              <div className="pt-4 flex items-center justify-between border-t border-slate-100">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-sm rounded-xl shadow-md shadow-indigo-200 transition-all flex items-center gap-2"
                >
              ...
              <div className="mt-6 space-y-4 animate-in fade-in duration-200">
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-2xs">
                  <button
                    ...
                    className="w-full px-5 py-3 bg-slate-50 hover:bg-slate-100/80 flex items-center justify-between text-left text-xs font-bold text-slate-700 transition-colors"
                  >
                  ...
                  {showExplanation && (
                    <div className="p-5 border-t border-slate-100 bg-white text-sm text-slate-800 leading-relaxed">
              ...
              {filteredQuestions.map((q, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 min-w-[32px] sm:w-9 sm:h-9 sm:min-w-[36px] aspect-square rounded-xl text-xs font-extrabold font-mono shrink-0 transition-all flex items-center justify-center relative cursor-pointer ${
                      isActive
                        ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30 ring-2 ring-indigo-400/50 scale-105 z-10'
                        : 'bg-slate-200/90 hover:bg-slate-300 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-100 border border-slate-300 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-400'
                    }`}
      ...
      {/* ========================================================
          QUESTION MATRIX MODAL (TOÀN BỘ CÂU HỎI TRONG CHUYÊN ĐỀ)
         ======================================================== */}
      {showMatrixModal && (
          ...
          <div className="bg-white dark:bg-[#11192b] rounded-3xl shadow-2xl max-w-xl w-full max-h-[85vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                    Bảng ma trận câu hỏi chuyên đề
                  </h3>
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-3 flex-wrap gap-2">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-md bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600"></span> Chưa chọn
                </span>
                <span className="flex items-center gap-1.5">
                ...
                {filteredQuestions.map((q, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-2xl border text-center transition-all relative flex flex-col items-center justify-center gap-1 group cursor-pointer ${
                        isActive
                          ? 'bg-indigo-600 text-white border-indigo-600 shadow-md ring-2 ring-indigo-400/40'
                          : 'bg-white dark:bg-[#131d31] border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 hover:border-indigo-400 hover:bg-indigo-50/40 dark:hover:bg-slate-800 shadow-2xs'
                      }`}
                    >
                      {isSaved && (
            ...
            {/* Modal Footer */}
            <div className="px-6 py-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-[#0c1424] flex justify-end">
              <button
                onClick={() => setShowMatrixModal(false)}
